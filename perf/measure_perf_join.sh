...
do
    ...
    do
        ...
        cmd="SELECT COUNT(col1) FROM './data/numbers_$left.csv' AS a JOIN './data/numbers_$right.csv' AS b ON b.col1>a.col2"
        /usr/bin/time -f "CSV;$left;$right;%E" ./build/squel "$cmd"
        echo "Start TDB-measures for join $left vs $right"
        cmd="SELECT COUNT(a.col1) FROM perftest_$left AS a JOIN perftest_$right AS b ON b.col1>a.col2"
