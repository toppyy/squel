...
@test "Simple WHERE \w Greater or equal than." {
    run ./build/squel "SELECT col1,col2,col3 FROM './test/data/small.csv' WHERE col3>=400"
    [[ $"${lines[1]}" == "UU;DEFG;999" ]]
    [[ $"${lines[2]}" == "DEFG;DEFG;400" ]]
}

@test "Simple WHERE \w Less or equal than." {
    run ./build/squel "SELECT col1,col2,col3 FROM './test/data/small.csv' WHERE col3<=100"
     [[ $"${lines[1]}" == "MAMA;ABC;32" ]]
     [[ $"${lines[2]}" == "UU;UU;100" ]]
     [[ $"${lines[3]}" == "ABC;ZYX;2" ]]
}
